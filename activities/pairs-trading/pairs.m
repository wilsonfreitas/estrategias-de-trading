...
for i = max(M,N) : N : length(s)-N
    ...
    h = cadf(series2(i-M+1:i, 1), series2(i-M+1:i, 2), 0, 0);
    if h.adf <= h.crit(2) % 5% conf. level
        % Only engage in trading if we reject the null hypothesis that no
        % cointegrating relationship exists.
        reg1 = ols(series2(i-M+1:i, 1), [ones(M,1) series2(i-M+1:i, 2)]);
        % The strategy:
        % 1. Compute residuals over next N days
        res = series2(i:i+N-1, 1) ...
            - (reg1.beta(1) + reg1.beta(2).*series2(i:i+N-1, 2));
        ...
        % opposite.
        rmse = std(reg1.resid);
        indicate(i:i+N-1) = res/rmse;
        
        s(i:i+N-1, 2) = (res/rmse > spread) - (res/rmse < -spread);
        s(i:i+N-1, 1) = -reg1.beta(2) .* s(i:i+N-1, 2);
    end
end
